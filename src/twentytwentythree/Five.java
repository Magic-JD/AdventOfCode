...
public class Five {
    ...
    private static class Range {
        ...
        public List<List<MR>> fallsInRange(MR mr) {
            long conStart = mr.start - sourceStart;
            long conEnd = mr.end - sourceStart;
            if (conEnd < 0 || conStart >= range) {
                return Collections.emptyList();
            }
            if (conStart >= 0 && conEnd < range) {
                return List.of(List.of(new MR(destStart + conStart, destStart + conEnd)), Collections.emptyList());
            }
            if (conStart >= 0) {
                long remainingRange = conEnd - range;
                return List.of(
                        List.of(new MR(destStart + conStart, destStart + range - 1)),
                        List.of(new MR(mr.start + (range - conStart), mr.end))
                );
            }
            if (conEnd < range) {
                long remainingRange = Math.abs(conStart);
                return List.of(
                        List.of(new MR(destStart, destStart + (mr.end - mr.start - remainingRange))),
                        List.of(new MR(mr.start, mr.start + remainingRange - 1))
                );
            } else {
                long remainingRange1 = conEnd - range;
                return List.of(
                        List.of(new MR(destStart, destStart + range - 1)),
                        List.of(new MR(mr.start, mr.start + Math.abs(conStart) - 1), new MR(mr.start + (range - conStart), mr.end))
                );
            }
        }
    }
    ...
    public static void run(List<String> input, String expectedOutput, long startTime) {
        List<Long> seeds = Arrays.stream(split(input.get(0).split(":")[1], " ")).map(LineUtils::extractLong).toList();
